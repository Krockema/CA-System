...
for (divide in seq(5, 15, 1)) {
	for (flip in seq(0, 10, 1)) {
		str <- paste("../CA-Daten/ca_heatmap/ca_divide_", divide,"_flip_", flip, ".txt", sep="")
...
for(i in seq(0, 120, 1)) {
	...
	str <- paste("../CA-Daten/rScriptData/ca_divide_", i %/% 11 + 5,"_flip_", i %% 11, ".txt", sep="")
	i = i + 1
...
str <- paste("../Skript-Daten/CSMassData.txt")
