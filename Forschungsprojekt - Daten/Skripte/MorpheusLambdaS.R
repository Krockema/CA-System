...
for (i in c(0, 0.5, 1.5, 2)) {
	str <- paste("../Morpheus-Daten/Veränderung der Variablen/LambdaS ", i, "/logger.txt", sep="")
	if (i == 0)
...
for (j in seq(1, 4)) {
	...
	#Plots
	str <- paste("../Plots/Variables/LambdaS/morpheus_cell_system_evaluation_round_", j, ".pdf", sep="")
	pdf(str)
	...
	str <- paste("../Skript-Daten/Variables/LambdaS/morpheusDataLambdaS ", j, ".txt", sep="")
	write.table(printDataMorpheusList[[j]], file = str, sep="\t")
	rm(printDataMorpheus)
}

#Daten gemeinsam darstellen
str <- paste("../Plots/Variables/LambdaS/morpheus_cell_system_evaluation.pdf", sep="")
pdf(str)
...
legend("bottomright", legend=c("0", "0.5", "1.5", "2"), col=c(1, 2, 3, 4), lty = c(3, 3, 3, 3), title="Werte für Lambda_S")
...
legend("topright", legend=c("0", "0.5", "1.5", "2"), col=c(1, 2, 3, 4), lty = c(3, 3, 3, 3), title="Werte für Lambda_S")
...
plot(abdeckungsPlot$Time, c(0, 0.5, 1.5, 2), lty=3, lwd=1, cex=0.5, ylab="Lambda_S", xlab="MCS bis 80% Abdeckung")
dev.off()

#printData in Datei überführen
str <- paste("../Skript-Daten/Variables/LambdaS/morpheusDataGoal.txt", sep="")
write.table(printDataMorpheusList[[j]], file = str, sep="\t")