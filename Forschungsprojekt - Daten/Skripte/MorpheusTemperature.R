...
for (i in c(-2, 0, 5, 7)) {
	str <- paste("../Morpheus-Daten/Veränderung der Variablen/Temperature ", i, "/logger.txt", sep="")
	if (i == -2)
...
for (j in seq(1, 4)) {
	...
	#Plots
	str <- paste("../Plots/Variables/Temperature/morpheus_cell_system_evaluation_round_", j, ".pdf", sep="")
	pdf(str)
	...
	str <- paste("../Skript-Daten/Variables/Temperature/morpheusDataTemperature ", j, ".txt", sep="")
	write.table(printDataMorpheusList[[j]], file = str, sep="\t")
	rm(printDataMorpheus)
}

#Daten gemeinsam darstellen
str <- paste("../Plots/Variables/Temperature/morpheus_cell_system_evaluation.pdf", sep="")
pdf(str)
...
legend("bottomright", legend=c("-2", "0", "5", "7"), col=c(1, 2, 3, 4), lty = c(3, 3, 3, 3), title="Werte für Temperatur")
...
legend("topright", legend=c("-2", "0", "5", "7"), col=c(1, 2, 3, 4), lty = c(3, 3, 3, 3), title="Werte für Temperatur")
...
plot(abdeckungsPlot$Time, c(-2, 0, 5, 7), lty=3, lwd=1, cex=0.5, ylab="Temperatur", xlab="MCS bis 80% Abdeckung")
dev.off()

#printData in Datei überführen
str <- paste("../Skript-Daten/Variables/Temperature/morpheusDataGoal.txt", sep="")
write.table(printDataMorpheusList[[j]], file = str, sep="\t")